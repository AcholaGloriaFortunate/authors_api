...
#         email = data.get('email')
#         user_type = data.get('user_type', 'author')  # Default to 'author'
#         password = data.get('password')
#         biography = data.get('biography', '') if user_type == 'author' else ''

#         # Basic input validation
#         if not all(isinstance(data.get(field), str) and data.get(field) for field in ['first_name', 'last_name', 'contact', 'password', 'email']):
#             return jsonify({'error': 'All fields are required and must be strings'}), 400

#         if user_type == 'author' and not isinstance(biography, str):
#             return jsonify({'error': 'Biography must be a string'}), 400
...
# from flask import Blueprint, request, jsonify
# from authors_app.extension import SQLAlchemy
...
#         op.add_column('user', sa.Column('user_column', sa.String(length=255), nullable=True))
...
# #auth blueprint
# auth = Blueprint('auth', __name__, url_prefix='/api/v1/auth')  # Fix the syntax for '__name__'
# bcrypt = Bcrypt()
...
#         required_fields = ['first_name', 'last_name', 'contact', 'password', 'email', 'biography']
#         if not all(request.json.get(field) for field in required_fields):
...
#         if User.query.filter_by(contact=contact).first() is not None:
#             return jsonify({'error': 'Contact already exists'}), TP_409_CONFLICT
...
#         #define variable that keeps track of user_name
#         username = f"{new_user.get_first_name} {new_user.get_last_name}"
...
#             }
#         }), HTTP_201_CREATED
#     except Exception as e:  # Move the except block to correct indentation
#         db.session.rollback()
